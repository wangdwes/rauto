...
pz = mwpsolver(n, r, [wpz, tau]);
[trajz, t_traj] = mwptraj({pz}, tau);

traj = [zeros(2, length(t_traj)); trajz{1}];
t_trajstart = 0;
t_trajend = 0;
ei_trajstart = 0;
ei_trajend = 0;
...
% 
% H = polyhess(n, r, tau);
% A = rescoeff(n, 0:r, 0:r, tau);
% b = [1 0 0 10 0 0]';
% 
% p = flip(quadprog(H, [], [], [], A, b));
% 
% t = linspace(0, tau);
% trajz = polyval(p, t);
% 
% traj = [zeros(2, length(t)); trajz];
% traj_ind = 1;
% ##########
...
handle = figure(9); % Take derivative of vel to get acceleration
for ip=1:3
    subplot(1,3,ip);
    hold on;
    acc = diff(vel_odom(ip,oi_trajstart:oi_trajend))./diff(odom_time(oi_trajstart:oi_trajend));
    acc_f = filter(1/4*ones(4,1), 1, acc);
    %plot(odom_time(oi_trajstart:oi_trajend-1), acc, colors(ip));
    plot(odom_time(oi_trajstart:oi_trajend-1), acc_f, colors(ip));
    xlabel('t');
    ylabel(sprintf('acc(%d)', ip), 'Interpreter', 'none');
    if ip == 2; title('acceleration', 'Interpreter', 'none'); end
end
if saveplot == 1
    saveas(handle, fullfile(plotdir, sprintf('q%s_acc.fig', qnum)));
    print(handle, '-depsc', fullfile(plotdir, sprintf('q%s_acc.epsc', qnum)));
end
